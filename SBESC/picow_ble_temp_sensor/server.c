...
#define LEN_GLICOSE_ARRAY 10
volatile uint8_t idx = 0;
const uint16_t glicose[] = {
    75,
    76,
    83,
    91,
...
static void packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case ATT_EVENT_CAN_SEND_NOW:
        printf("Notificação BLE enviada: glicose[%d] = %u\n", idx, glicose[idx]);
        att_server_notify(con_handle, ATT_CHARACTERISTIC_ORG_BLUETOOTH_CHARACTERISTIC_TEMPERATURE_01_VALUE_HANDLE, (uint8_t *)&glicose[idx], sizeof(uint16_t));
        idx++;
        if (idx >= LEN_GLICOSE_ARRAY)
            idx = 0;
        break;
    default:
        break;
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    {
        return att_read_callback_handle_blob((const uint8_t *)&glicose[idx], sizeof(uint16_t), offset, buffer, buffer_size);
    }
    return 0;
}
